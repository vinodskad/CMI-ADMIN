...
export default function MultiSelectInput({value,label,isRequired,onChange, dataSet, getOptionLabel,getOptionValue}) {
  ...
  useEffect(()=>{
   ...
   var selectedArr=[]
    selected?.map((item)=>{
      return selectedArr.push({id:item});
    });
 ...
 // onChange(selected)
  ...
  // useEffect(()=>{
  //   if(value){
  //    var setSeleceted=value?.map(x=>x?.[getOptionValue]);
      ...
      <FormControl sx={{ width: "100%", }}>
        ...
        <Select
         ...
         // renderValue={(selected) => selected.join(", ")}
          renderValue={(selected) => selected?.map(obj=> dataSet[obj - 1]?.name)?.join(", ")}
        >
          {dataSet?.map((name) => (
            <MenuItem key={name.id} value={name[getOptionValue]}>
              <Checkbox checked={selected.indexOf(name[getOptionValue]) > -1} />
